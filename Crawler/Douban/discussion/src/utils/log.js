...
log4js.configure({
    appenders: {
        ...
        group: {  // 文件形式打印日志
            type: 'dateFile',
            filename: path.join(__dirname, '../../logs/group/group'), // 写入日志文件的路径
        ...
        discussion: {  // 文件形式打印日志
            type: 'dateFile',
            filename: path.join(__dirname, '../../logs/discussion/discussion'), // 写入日志文件的路径
            alwaysIncludePattern: true, // （默认为false） - 将模式包含在当前日志文件的名称以及备份中
            pattern: '.yyyy-MM-dd.log', // （可选，默认为.yyyy-MM-dd） - 用于确定何时滚动日志的模式。格式:.yyyy-MM-dd-hh:mm:ss.log
            encoding: 'utf-8', // default 'utf-8'，文件的编码
            maxLogSize: 1024 * 1024, //文件最大存储空间，当文件内容超过文件存储空间会自动生成一个文件xxx.log.1的序列自增长的文件
...
exports.group = log4js.getLogger('group')
exports.discussion = log4js.getLogger('discussion')
